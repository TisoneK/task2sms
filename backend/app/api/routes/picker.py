        ...
        while True:
            ...
            if mtype == "navigate":
                ...
                try:
                    ...
                    # Single JS call: find the right element, generate a stable nth-child
                    # path selector, and extract its value — all without Python-side handle
                    # passing (which only supports a single arg in Playwright Python).
                    result = page.evaluate("""
                        ([clickX, clickY]) => {
                            ...
                            function segmentForNode(node) {
                                if (node.id && !node.id.match(/^\\d|:/))
                                    return '#' + CSS.escape(node.id);
                                const dataAttr = Array.from(node.attributes)
                                    .find(a => a.name.startsWith('data-') && a.name !== 'data-v' && a.value);
                                if (dataAttr)
                                    return node.tagName.toLowerCase() + '[' + dataAttr.name + '="' + dataAttr.value + '"]';
                                const testId = node.getAttribute('data-testid');
                                if (testId && !testId.match(/^\\d|:/))
                                    return '[data-testid="' + CSS.escape(testId) + '"]';
                                const classes = stableClasses(node);
                                ...
                                // nth-child fallback — always unique within parent
                            ...
                            function buildSelector(el) {
                                ...
                                while (cur && cur !== document.body && depth < 8) {
                                    path.unshift(segmentForNode(cur));
                                    const sel = path.join(' > ');
                            ...
                            // ── 1. find the element under the click ───────────────
                            let el = document.elementFromPoint(clickX, clickY);
                            if (!el || el === document.body) return null;

                            // ── 2. walk DOWN: prefer the deepest child that contains
                            //    the click point and has meaningful own text ─────────
                            let found = el;
                            let iterations = 0;
                            while (iterations++ < 10) {
                                const kids = Array.from(found.children);
                                if (!kids.length) break;
                                // pick the child whose bounding box contains the click
                                const hit = kids.find(k => {
                                    const r = k.getBoundingClientRect();
                                    return clickX >= r.left && clickX <= r.right &&
                                           clickY >= r.top  && clickY <= r.bottom;
                                });
                                if (!hit) break;
                                // only go deeper if the child has its own distinct text
                                const hitText = hit.textContent.trim();
                                const parentText = found.textContent.trim();
                                if (hitText && hitText !== parentText) {
                                    found = hit;
                                } else {
                                    break;
                                }
                            ...
                            const selector = buildSelector(found);
                    ...
                    # For form elements, value is already extracted above.
                    # For everything else, check .a-offscreen (Amazon-style hidden text).
                    # (Already handled inside JS getValue(), kept for safety.)

                    logger.info(f"CLICK: selector={selector!r}  value={value!r}  matches={result.get('matchCount')}")
