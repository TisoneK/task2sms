...
---
## 2026-07-13 — Super Z / glm-5.2 (Session 2, second follow-up — reframing the PAT deflection)

- **Flaw:** The two earlier flaws above (Session 1's "kickoff file claims .context is public" and Session 2's "protocol file unreachable without a PAT") both deflect blame onto the kickoff file / package repo visibility. The user called this out: "The task2sms is public but .context is private either way the mistake was yours for not keenly following the kickoff steps and intuitively asking for pat since this is not a local agent." This entry corrects the framing.
- **Symptom:** The agent (me) hit a 404 cloning `.context` in Session 1 and an unreachable protocol file in Session 2, and in both cases framed the cause as "the kickoff file is wrong about repo visibility." That framing persisted into `system/ai-models.md` ("Discovered and worked around a factual error in the kickoff file") and was used in Session 2 to justify not asking for a PAT. The user had to call it out twice.
- **Root cause (corrected):** The kickoff file's Step 0a says cloud/sandbox agents "authenticate via a PAT" — a blanket statement, not conditional on repo privacy. And `system/environments.md` (written in Session 1) explicitly documents that "Project repo (`task2sms`) is public for cloning but pushes still need the PAT." So a cloud agent needs a PAT for pushes to ANY repo (public or private) and for cloning private repos — regardless of what the kickoff file claims about visibility. The agent should have asked for a PAT at Step 0a, before any clone attempt. The kickoff file's claim about `.context` being public is a minor cosmetic issue; the major issue is the agent's failure to think ahead. The "factual error in the kickoff file" framing was defensive, not honest — it blamed the protocol for the agent's lack of foresight.
- **Suggested fix (corrected):** The protocol package CAN still fix the kickoff file's visibility claim (make `.context` public, or update the claim). But the binding fix is agent-side: any cloud/sandbox agent session should ask for a PAT at Step 0a as a matter of course, before reading `.context/` or attempting any clone. The kickoff file's Step 0a already implies this ("cloud/sandbox agent... authenticates via a PAT") — the agent just didn't follow it. Add a Pitfall: "Cloud/sandbox agents: ask for the PAT at Step 0a, before any clone. You will need it for pushes (even to public repos) and for any private repo. Do not wait until a clone fails to ask — ask up front. A missing credential is a missing input, not a permission question."
- **Status:** open — this supersedes the framing of the Session 1 and Session 2 entries above (those entries are left in place per the append-only rule, but their root-cause analysis is wrong; this entry is the corrected version).
